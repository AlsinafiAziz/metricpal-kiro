    ...
    const sendData = method => {
        ...
        const payload = {
            ...
            actionLog: actionLogCopy.map(event => ({
                ...
                url: event.url,
                element: event.action.actionElement || null,
                text: event.action.actionText || null,
                value: event.action.actionValue || (event.action.formData ? JSON.stringify(event.action.formData) : null),
                properties: event.action.actionProperties || null
            })),
            referrer: referrer
        };
